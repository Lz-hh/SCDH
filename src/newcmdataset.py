...
class CMDataset(data.Dataset):
    ...
    def open_data(self):
        ...
        elif self.data_name.lower() == 'new_dataset':
            data = new_dataset_features(self.partition)
...
def MIRFlickr25K_fea(partition):
    root = 'D:/Datasets/data/MIRFLICKR25K/'
...
def new_dataset_features(partition):
    root = '/path/to/your/dataset'
    if partition.lower() == 'train':
        filename = 'train_features'
    else:
        filename = 'test_features'
    feature_file = os.path.join(root, filename)
    features = []
    with open(feature_file, 'rb') as f:
        while True:
            try:
                feature = np.load(f)['g_feature']
                features.append(feature)
            except:
                break
    features = np.array(features)
    texts = [] # replace with your code to load text data
    labels = [] # replace with your code to load labels
